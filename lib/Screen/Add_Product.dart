...
class AddProduct extends StatefulWidget {
  bool? isEdit;
  Product? model;
  AddProduct({Key? key, this.isEdit, this.model}) : super(key: key);
...
//                        Parameter For API Call
  ...
  String? description; // pro_input_description
  String? selectedCatID; //category_id
  String productType= 'simple_product'; //product_type
...
// logic clear
TextEditingController sortDescriptionCon = TextEditingController();
      ...
      children: [
        InkWell(
          onTap: () {
            ...
            print("selectedCatID is $selectedCatID");
            getCat();
...
// logic painding
        ...
        children: [
          ...
          InkWell(
            ...
            onTap: () {
              ...
              // Navigator.push(context, MaterialPageRoute(builder: (context) => Media(from: "main"),),);
            },
          ),
        ],
      ),
  ...
  selectedMainImageShow() {
    return (selectedimage == null || selectedimage == "") && (myImage != null || myImage != '')
        ? Container()
...
// logic painding
        ...
        children: [
          ...
          InkWell(
            ...
            onTap: () {
              ...
              // Navigator.push(context, MaterialPageRoute(builder: (context) => Media(from: from, pos: pos,),));
              //otherImagesFromGallery();
            },
          ),
        ],
  ...
  uploadedOtherImageShow() {
    return
      (selectedimage2 == null || selectedimage2 == "") && (otherImage != null || otherImage != '') ? Container() :
                  ...
                  children: [
                    productType != null
                      ...
                      productType == 'simple_product'
                        ...
                        // : Text(getTranslated(context, "Select Type")!,
                        : Text( getTranslated(context, "Simple Product")!,
                    ),
                  ],
                ),
              ),
...
//=========================== Add Product API Call =============================
  ...
  Future<void> addProductAPI(List<String> attributesValuesIds) async {
    ...
    bakery_id = prefs.getString('bakery_id');
    print("bakery_id__________${bakery_id}");
    if (_isNetworkAvail && (selectedimage != null || selectedimage != '')) {
      try {
        var request = http.MultipartRequest("POST", addProductsApi);
        request.headers.addAll(headers);

        if(widget.isEdit == true)
        request.fields['edit_product_id'] = "${widget.model?.id}";

        request.fields[SellerId] = bakery_id!;
        request.fields['sub_category_id'] = widget.isEdit==true && (sub_cat_id == null || sub_cat_id == '') ? "${widget.model?.categoryId}" : sub_cat_id.toString();
        request.fields[ProInputName] = widget.isEdit==true && (productName == null || productName == '') ? "${widget.model?.name}" : "${productName}" ;
        request.fields[ShortDescription] =widget.isEdit==true && (sortDescription == null || sortDescription == '') ? '${widget.model?.shortDescription}' :'$sortDescription' ;
        // if (tags != null) request.fields[Tags] = tags!;
        // if (taxId != null) request.fields[ProInputTax] = taxId!;
        // if (indicatorValue != null) request.fields[Indicator] = indicatorValue!;
        // if (madeIn != null) request.fields[MadeIn] = madeIn!;
        ...
        if (selectedimage != null) {
          request.files.add(await http.MultipartFile.fromPath(ProInputImage, widget.isEdit==true && (selectedimage == null || selectedimage == '') ? "${widget.model?.image}" :selectedimage.path));
        }
        if (selectedimage2 != null) {
          request.files.add(await http.MultipartFile.fromPath(OtherImages,  widget.isEdit==true && (selectedimage2 == null || selectedimage2 == '') ? "${widget.model?.otherImage}" : selectedimage2.path));
        }

        print('add prod request${request.fields} ${request.files} $selectedimage $selectedimage2');
        ...
        //   request.fields[ProInputVideo] = uploadedVideoName;
        // }
        request.fields[ProInputDescription] = widget.isEdit==true && (description == null || description == '') ? "${widget.model?.desc}" : description!;
        request.fields[CategoryId] =widget.isEdit==true && (selectedCatID == null || selectedCatID == '') ? "${widget.model?.categoryId}" : '$selectedCatID';
        //attribute_values
        // this is complecated
        request.fields[ProductType] =widget.isEdit==true && (productType == null || productType == '') ? "${widget.model?.type}" :  productType!;
        // request.fields[VariantStockLevelType] = variantStockLevelType!;
        // request.fields[AttributeValues] = attributesValuesIds.join(",");
        // if(product_type == variable_product)
        //simple product
        if (productType == 'simple_product') {
          ...
          if (_isStockSelected == null)
            ...
            status = simpleproductStockStatus;
          request.fields[SimpleProductStockStatus] = status ?? 'null';
          request.fields[SimplePrice] =widget.isEdit==true && (simpleProductPriceController.text == null || simpleProductPriceController.text == '') ? "${widget.model?.prVarientList?[0].price}" :  simpleProductPriceController.text;
          request.fields[SimpleSpecialPrice] =widget.isEdit==true && (simpleProductSpecialPriceController.text == null || simpleProductSpecialPriceController.text == '') ? "${widget.model?.prVarientList?[0].disPrice}" :
              simpleProductSpecialPriceController.text;
          if (_isStockSelected != null &&
              _isStockSelected == true &&
              simpleproductSKU != null) {
        ...
        print("response_is________${getdata['data']}");
        if (!error) {
          await buttonController!.reverse();
  ...
  autoFill() async{
    // print("autofill_checking_____________ ${widget.model?.otherImage?[0]} ${widget.model?.otherImage}");
    setState((){
      loading = true;
    });
    productNameControlller.text = "${widget.model?.name}";
    sortDescriptionCon.text = "${widget.model?.shortDescription}";
    selectedCatName = "${widget.model?.catName}";
    myImage = "${widget.model?.image}";
    if(widget.model?.otherImage?.length != 0 )
    otherImage = "${widget.model?.otherImage?[0]}";
    descCon.text = "${widget.model?.desc}";
    if(widget.model?.prVarientList?.length != 0 ) {
      simpleProductPriceController.text = "${widget.model?.prVarientList?[0].price}";
      simpleProductSpecialPriceController.text = "${widget.model?.prVarientList?[0].disPrice}";
      // simpleProductSKUController.text = "${widget.model?.prVarientList?[0].sku??""}";
      // simpleProductTotalStock.text = "${widget.model?.prVarientList?[0].stock??''}";
    }
    // _isStockSelected = widget.model?.stockType != null && widget.model?.stockType.toString() == "1" ?  true : false;
...
//=========================== Body Part ========================================
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            mainImage(), //only API panding
            selectedMainImageShow(),
            (selectedimage == null || selectedimage == "") && (myImage == null || myImage == '') ? Container() :
            (selectedimage == null || selectedimage == "") && (myImage != "" || myImage != null  ) && widget.isEdit == true?
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 10),
              child: ClipRRect(
                borderRadius:
            ...
            uploadedOtherImageShow(),
            (selectedimage2 == null || selectedimage2 == "") && (otherImage == null || otherImage == '') ? Container() :
           (selectedimage2 == null || selectedimage2 == "") && (otherImage != "" || otherImage != null  ) && widget.isEdit == true?
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 10),
                child: ClipRRect(
                  borderRadius:
            ...
            Center(
              child: AppBtn(
                title:widget.isEdit==true? "Edit Product": getTranslated(context, "Add Product")!,
                btnAnim: buttonSqueezeanimation,
                btnCntrl: buttonController,
                onBtnSelected: () async {
                  validateAndSubmit();
  ...
  bool validateAndSave() {
    ...
    form.save();
    if (form.validate()) {
      if (productType == null && widget.isEdit != true) {
        setSnackbar(
          getTranslated(context, "Please select product type")!,
        );
        return false;
      ...
      //   return false;
      // }
      else if (selectedCatID == null && widget.isEdit != true) {
        setSnackbar(getTranslated(context, "Please select category")!);
        return false;
      } else if (selectedTypeOfVideo != null && videoUrl == null && widget.isEdit != true) {
        setSnackbar(
          getTranslated(context, "Please enter video url")!,
        );
        return false;
      } else if (productType == 'simple_product' && widget.isEdit != true) {
        if (simpleProductPriceController.text.isEmpty) {
          setSnackbar(
            getTranslated(context, "Please enter product price")!,
          );
          return false;
            ...
            simpleProductSpecialPriceController.text.isNotEmpty &&
            double.parse(simpleProductSpecialPriceController.text) >
                double.parse(simpleProductPriceController.text) && widget.isEdit != true) {
          setSnackbar(
            getTranslated(context, "Special price can not greater than price")!,
          );
          return false;
        }
        // else if (_isStockSelected != null && _isStockSelected == true && widget.isEdit != true) {
        //   if (simpleproductSKU == null || simpleproductTotalStock == null && widget.isEdit != true) {
        //     setSnackbar(
        //       getTranslated(context, "Please enter stock details")!,
        //     );
        //     return false;
  ...
  Widget build(BuildContext context) {
    ...
    width = MediaQuery.of(context).size.width;
    return Scaffold(
        appBar: getAppBar(widget.isEdit==true? "Edit Product":
          getTranslated(context, "Add New Product")!, context,),
        body:
  ...
  Future<CategoryModel?> getCat() async {
    ...
    request.fields.addAll({'seller_id': '$CUR_USERID', "category_id": selectedCatID.toString()??''});
    ...
    if (response.statusCode == 200) {
      ...
      if(json.decode(str)['message'].toString() == "Category does not exist")
        toast("Sub Category does not exist, please add sub category.");

      sub_cat_id = json.decode(str)['data'][0]['id'];

      print("response_issssssss${sub_cat_id}");
      print("response_issssssss${json.decode(str)}");
